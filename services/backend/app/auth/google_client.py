...
# Local dev redirects over plain http://localhost; oauthlib refuses to
# exchange tokens over a non-https redirect URI unless this is set.
...
def make_flow(scopes: list[str] | None = None):
    ...
    # PKCE's code_verifier is generated per-Flow-instance and we build a
    # fresh Flow in /auth/start and another in /auth/callback, so an
    # autogenerated verifier from /auth/start is never available to the one
    # that exchanges the code. This is a confidential client (has a
    # client_secret), so PKCE isn't required here; disable it rather than
    # trying to persist the verifier across requests.
